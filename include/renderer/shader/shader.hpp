...
concept ValidUniformSpec = (Size <= 4)
                           && (std::same_as<Type, unsigned int> || std::same_as<Type, float> || std::same_as<Type, int>
...
public:
  [[nodiscard]] constexpr auto GetShaderID() const noexcept -> unsigned int { return m_ShaderID; }
  ShaderUnit(const ShaderUnit &other) = delete;
  ShaderUnit(ShaderUnit &&) = default;
  ShaderUnit &operator=(const ShaderUnit &) = delete;
  ShaderUnit &operator=(ShaderUnit &&) = default;
  constexpr explicit ShaderUnit(std::string_view source) : m_ShaderID{ glCreateShader(ShaderType) }
  {
    // NOLINTNEXTLINE
    ...
    if (CompiledSuccessfully != GL_TRUE) {
      ...
      // NOLINTNEXTLINE
      glGetShaderInfoLog(m_ShaderID, NULL, nullptr, reinterpret_cast<GLchar *>(Message.data()));
...
template<typename T>
concept ShaderType =
  std::same_as<T, ShaderUnit<GL_COMPUTE_SHADER>> || std::same_as<T, ShaderUnit<GL_VERTEX_SHADER>>
  || std::same_as<T, ShaderUnit<GL_TESS_CONTROL_SHADER>> || std::same_as<T, ShaderUnit<GL_TESS_EVALUATION_SHADER>>
  || std::same_as<T, ShaderUnit<GL_GEOMETRY_SHADER>> || std::same_as<T, ShaderUnit<GL_FRAGMENT_SHADER>>;
// Function to add all shader units to program

void AddShaderUnits(unsigned int program_id, renderer::gl::ShaderType auto &&first)
{
...
public:
  template<renderer::gl::ShaderType... Args> explicit Program(Args... shader_units)
  {
    ...
    if (ProgramLinked != GL_TRUE) {
      ...
      Message.reserve(static_cast<std::size_t>(LogLength));
      // NOLINTNEXTLINE
      glGetProgramInfoLog(m_ProgramID, NULL, nullptr, reinterpret_cast<GLchar *>(Message.data()));
    ...
    requires(ValidUniformSpec<Size, Type> && (Size == 1))
  {
    ...
    if (Location == -1) { throw renderer::UniformError(fmt::format("\"{}\"", name)); }
  ...
  template<std::size_t Size, typename Type>
  void SetUniform(std::string const &name, Type value1, Type value2, Type value3)
  ...
  template<std::size_t Size, typename Type>
  void SetUniform(std::string const &name, Type value1, Type value2, Type value3, Type value4)
    requires(ValidUniformSpec<Size, Type> && (Size == 4))
  {
    auto Location = glGetUniformLocation(m_ProgramID, name.data());
    if (Location == -1) { throw renderer::UniformError(fmt::format("\"{}\" was not found", name)); }
    if constexpr (std::same_as<Type, float>) {
  ...
  auto UseProgramInFunction(Func &&function, Params... parameters) noexcept(noexcept(std::forward<Func>(function)))
  {
    return std::invoke(std::forward<Func>(function), m_ProgramID, std::forward<Params...>(parameters...));
  }
